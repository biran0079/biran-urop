...
public class PRCurve {
  ...
  public PRCurve(String filename) {
    try {
      ...
      for(String line : new FileLines(filename)) {
        List elems = StringUtils.split(line);
        Pair<Double, Integer> p = new Pair<Double, Integer>(new Double(elems.get(0).toString()), new Integer(elems.get(1).toString()));
        dataScores.add(p);
      }
      init(dataScores);
    } catch (Exception e) {
  ...
  public PRCurve(String filename, boolean svm) {
    try {
      ...
      for(String line : new FileLines(filename)) {
        ...
        double score = new Double(elems.get(1).toString()).doubleValue() + 0.5;
        Pair<Double, Integer> p = new Pair<Double, Integer>(new Double(score), new Integer(cls));
        dataScores.add(p);
      }
      init(dataScores);
    } catch (Exception e) {
  ...
  public void init(List<Pair<Double, Integer>> dataScores) {
    ...
    for (int i = 0; i < dataScores.size(); i++) {
      q.add(new Pair(new Integer(i), dataScores.get(i)), -dataScores.get(i).first().doubleValue());
    }
    ...
    for (int i = 0; i < sorted.size(); i++) {
      Pair<Double, Integer> next = (Pair<Double, Integer>) (((Pair) sorted.get(i)).second());
  ...
  public void initMC(ArrayList<Triple<Double, Integer, Integer>> dataScores) {
    ...
    for (int i = 0; i < sorted.size(); i++) {
      Triple<Double, Integer, Integer> next = (Triple<Double, Integer, Integer>) (((Pair) sorted.get(i)).second());
      scores[i] = next.first().doubleValue();
      classes[i] = next.second().intValue();
