...
public class LogConditionalObjectiveFunction extends AbstractStochasticCachingDiffFunction {
  ...
  public void calculateStochasticFiniteDifference(double[] x,double[] v, double h, int[] batch){
    ...
    double priorFactor = ((double) batch.length )/(((double) data.length)*prior.getSigma()*prior.getSigma());
  ...
  public void calculateStochasticGradientOnly(double[] x, int[] batch) {
    ...
    double priorFactor = ((double) batchSize)/(((double) data.length)*prior.getSigma()*prior.getSigma());
