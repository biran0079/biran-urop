package edu.stanford.nlp.optimization;


/**
 * <p>
 * Hybrid Minimizer is set up as a combination of two minimizers.  The first minimizer will ideally
 * quick convergence regardless of proximity to the true minimium, while the second minimizer would
 * generally be a Quadratic method, that is only fully quadratic near the solution.
 *
 * If you read this, send me an e-mail saying, "Alex!  You should finish adding the description to the Hybrid Minimizer!"
 *
 * @author <a href="mailto:akleeman@stanford.edu">Alex Kleeman</a>
 * @version 1.0
 * @since 1.0
 */
public class HybridMinimizer implements Minimizer{

  Minimizer firstMinimizer = new SMDMinimizer();
  Minimizer secondMinimizer = new QNMinimizer(15);
  int iterationCutoff = 1000;

  public HybridMinimizer(Minimizer minimizerOne, Minimizer minimizerTwo, int iterationCutoff){
    this.firstMinimizer = minimizerOne;
    this.secondMinimizer = minimizerTwo;
    this.iterationCutoff = iterationCutoff;
  }

  public double[] minimize(Function function, double functionTolerance, double[] initial) {
			return minimize(function, functionTolerance, initial, -1);
	}


  public double[] minimize(Function function, double functionTolerance, double[] initial, int maxIterations) {

    double[] x;
    x = firstMinimizer.minimize(function,functionTolerance,initial,iterationCutoff);

    x = secondMinimizer.minimize(function,functionTolerance,x,maxIterations);

    return x;
  }
}

