...
public class ScaledSGDMinimizer extends StochasticMinimizer {
  ...
  public double tuneFixedGain(edu.stanford.nlp.optimization.Function function, double[] initial, long msPerTest,double fixedStart){
    ...
    do{
      ...
      System.err.println("");
      this.fixedGain = f;
      System.err.println("Testing with batchsize: " + this.bSize + "    gain:  " +  this.gain + "  fixedGain:  " + nf.format(fixedGain) );
  ...
  public Pair<Integer,Double> tune( edu.stanford.nlp.optimization.Function function,double[] initial, long msPerTest){
    ...
    for(int i =0;i<2; i++){
      ...
      this.gain = tuneGain(function,initial,msPerTest,1e-7,1.0);
      this.bSize= tuneBatch(function,initial,msPerTest,1);
      System.err.println("Results:  fixedGain: " + nf.format(this.fixedGain) + "  gain: " + nf.format(this.gain) + "  batch " + this.bSize );
    }
  ...
  private void updateDiagMinErr(double[] diag,double[] s,double[] y){
    ...
    say("M");
    alpha = Math.sqrt((ArrayMath.norm(y)/ArrayMath.norm(s))) *Math.sqrt(( 50.0/ (50.0 + (double) k) ));
