 ...
 *
 * QNMinimizer qn = new QNMinimizer(mem,true) mem - the number of pairs to
...
public class QNMinimizer implements Minimizer<DiffFunction> {
  ...
  public class Record {
    ...
    /*
     *
     * eStat toContinue() This function checks for convergence through first
     * order optimality, and numerical convergence (i.e. zero numerical
  ...
  public class QNInfo {
    ...
    public int update(double[] newS, double[] newY, double yy, double sy,
      ...
      try {
        ...
        System.err
            .println("Either convergence, or floating point errors combined with extremely linear region");
      }
  ...
  public double[] minimize(DiffFunction dfunction, double functionTolerance,
    ...
    sayln("Total time spent in optimization: " + nf.format(completionTime));
  ...
  private double[] lineSearchMinPack(DiffFunction dfunc, double[] dir,
    ...
    do {
      ...
      if ((bracketed && (newPt[a] <= stpMin || newPt[a] >= stpMax))
          || infoc == 0) {
        info = 6;
        say("linesearch failure: bracketed but no feasible found");
      }
      if (newPt[a] == aMax && newPt[f] <= fTest && newPt[g] <= gTest) {
        info = 5;
        say("linesearch failure: sufficient decrease, but gradient is more negative");
      }
      if (newPt[a] == aMin && (newPt[f] > fTest || newPt[g] >= gTest)) {
        info = 4;
        say("linesearch failure: minimum step length reached");
      }
      if (fevals >= maxFevals) {
        info = 3;
        throw new MaxEvaluationsExceeded(
      ...
      if (bracketed && stpMax - stpMin <= tol * stpMax) {
        info = 2;
        say("linesearch failure: interval is too small");
      }
