...
public abstract class AbstractStochasticCachingDiffFunction extends AbstractCachingDiffFunction {
  ...
  public SamplingMethod sampleMethod = SamplingMethod.RandomWithOutReplacment ;
  /**
   ...
   */
  abstract protected void calculateStochastic(double[] x, double[] v, int[] batch);
  ...
  /**
   * getBatch is used to generate the next sequence of indicies to be passed to the actual function.
   *  Depending on the current sample method this is done by:
   *    Ordered - simply generates the indicies 1,2,3,4,....
   *    RandomWithReplacment - Samples uniformly from the set of possible indicies
   *    RandomWithourReplacement - Samples from the set of possible indicies removing each used index, then restarting
   *          after each pass
...
//  private int numCalls = 0;
  private void getBatch(int batchSize){
...
//     numCalls++;
    ...
    //-----------------------------
    if (sampleMethod.equals(SamplingMethod.RandomWithReplacment)){
...
//        System.err.println("numCalls = "+(numCalls++));
      ...
      //-----------------------------
    }else if(sampleMethod.equals(SamplingMethod.RandomWithOutReplacment)){
  ...
  void stochasticEnsure(double[] x, double[] v, int batchSize){
    ...
    if (lastBatch == null){
      ...
      System.out.println("Setting last batch");
    }
  ...
  */
  ...
  private void getHdotVFiniteDifference(double[] x, double[] v, double[] curDerivative){
    ...
    for( int i = 0;i<x.length;i++){
      ...
      HdotV[i] = hInv*(derivative[i]-curDerivative[i]);
    }
